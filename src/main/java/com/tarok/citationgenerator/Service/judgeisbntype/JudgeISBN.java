...
public class JudgeISBN {
    public InputtedISBNType judge(String inputData) {
        int length = inputData.length();

        if (!(length == 10 || length == 13)) return InputtedISBNType.NOT_ISBN;

        if (length == 10) {
            var patternTwoByte = Pattern.compile("[０-９]{10}");
            var matcherTwoByte = patternTwoByte.matcher(inputData);
            if(matcherTwoByte.matches()) return InputtedISBNType.TWO_BYTE_ISBN;

            var patternOneByte = Pattern.compile("\\d{10}");
            var matcherOneByte = patternOneByte.matcher(inputData);
            if(matcherOneByte.matches()) return InputtedISBNType.ONE_BYTE_ISBN;
        }
        ...
        return InputtedISBNType.NOT_ISBN;
    }
    /**
     * 全角のISBNを半角に変更する
    ...
    public String normalizeByteType(String inputtedString) {
        ...
        InputtedISBNType result = judge.judge(inputtedString);

        //フォームのValidationではじかれるはずなのでエラーを出力する
        if (result == InputtedISBNType.NOT_ISBN) {
            log.warn("ISBNに不正な入力値");
            throw new IllegalArgumentException("isbnではありません");
        }

        //半角であるときはそのままつかえる
        if (result == InputtedISBNType.ONE_BYTE_ISBN) {
